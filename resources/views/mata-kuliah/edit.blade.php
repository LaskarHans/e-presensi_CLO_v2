...
<head>
    ...
    <title>Edit {{ $mataKuliah->nama }} · E-Hadir</title>
...
<nav class="sticky top-0 z-30 bg-gray-950/90 backdrop-blur border-b border-gray-800">
    <div class="max-w-3xl mx-auto px-6 h-16 flex items-center gap-3">
        ...
        <span class="text-gray-700">/</span>
        <span class="text-sm text-gray-100">Edit</span>
    </div>
</nav>
...
<main class="max-w-3xl mx-auto px-6 py-8">
    <div class="mb-6">
        ...
        <p class="text-gray-500 mt-1">{{ $mataKuliah->nama }} · {{ $mataKuliah->kode }}</p>
    </div>

    @if ($mataKuliah->presensis_count ?? false)
        <div class="mb-5 bg-amber-500/10 border border-amber-500/30 text-amber-300 text-sm rounded-lg p-4">
            Jadwal ini sudah punya {{ $mataKuliah->presensis_count }} catatan presensi. Mengubah hari/jam tidak akan mengubah data presensi yang sudah tercatat.
        </div>
    @endif

    <div class="bg-gray-900 border border-gray-800 rounded-xl p-6">
        @include('mata-kuliah._form', [
            'action' => route('mata-kuliah.update', $mataKuliah),
            'method' => 'PUT',
            'submitLabel' => 'Simpan Perubahan',
            'showSimpanLagi' => false,
