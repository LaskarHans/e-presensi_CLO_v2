...
<nav class="sticky top-0 z-30 bg-gray-950/90 backdrop-blur border-b border-gray-800">
    <div class="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between gap-4">
        ...
        <div class="hidden md:flex items-center gap-1 bg-gray-900/60 rounded-lg p-1">
            <button data-tab="dasbor" class="tab-btn relative px-4 py-1.5 rounded-md text-sm font-medium text-gray-400 transition">Dasbor</button>
            <button data-tab="tandai" class="tab-btn relative px-4 py-1.5 rounded-md text-sm font-medium text-gray-400 transition">
                Tandai Kehadiran
        ...
        <div class="flex items-center gap-3 shrink-0">
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="text-sm font-medium text-gray-400 hover:text-white transition">
                    Logout
                </button>
            </form>
        </div>
...
<main class="max-w-7xl mx-auto px-6 py-8">
    ...
    <section id="tab-dasbor" class="tab-section space-y-6">
        <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <div>
                ...
                <p class="text-gray-500 mt-1">{{ $user->prodi ?? '-' }} @if($user->angkatan) · Angkatan {{ $user->angkatan }} @endif</p>
            </div>
            <button type="button" onclick="switchTab('tandai')" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-500 transition text-white font-medium px-5 py-2.5 rounded-lg shrink-0">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="bg-gray-900 border border-gray-800 rounded-xl p-5">
                <div class="w-9 h-9 rounded-full bg-blue-500/10 text-blue-400 flex items-center justify-center mb-4">
            ...
            <div class="bg-gray-900 border border-gray-800 rounded-xl p-5">
                <div class="w-9 h-9 rounded-full bg-emerald-500/10 text-emerald-400 flex items-center justify-center mb-4">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                </div>
            ...
            <div class="bg-gray-900 border border-gray-800 rounded-xl p-5">
                <div class="w-9 h-9 rounded-full bg-purple-500/10 text-purple-400 flex items-center justify-center mb-4">
                    ...
                    @forelse($rekapPerMatkul as $r)
                        <div>
                            <div class="flex items-center justify-between text-sm mb-1.5">
                                <p class="font-medium">{{ $r->nama }} <span class="text-gray-500 font-mono text-xs">{{ $r->kode }}</span></p>
                                <span class="text-gray-500">{{ $r->hadir }}/{{ $r->total }}</span>
                            </div>
                            <div class="h-2 bg-gray-800 rounded-full overflow-hidden">
                                <div class="h-full rounded-full {{ $r->persentase >= 90 ? 'bg-emerald-500' : ($r->persentase >= 75 ? 'bg-amber-500' : 'bg-red-500') }}" style="width: {{ $r->persentase }}%"></div>
                            </div>
                            <p class="text-right text-xs mt-1 {{ $r->persentase >= 90 ? 'text-emerald-400' : ($r->persentase >= 75 ? 'text-amber-400' : 'text-red-400') }}">{{ $r->persentase }}%</p>
                        </div>
                    @empty
                ...
                <div class="space-y-3">
                    @forelse($kelasHariIni as $k)
                        <div class="border-l-2 border-blue-500 bg-gray-800/40 rounded-r-lg px-4 py-3">
                            ...
                            <p class="text-xs text-gray-500 mt-0.5">{{ \Carbon\Carbon::parse($k->jam_mulai)->format('H.i') }} – {{ \Carbon\Carbon::parse($k->jam_selesai)->format('H.i') }} · {{ $k->ruang }}</p>
                        </div>
                    @empty
                    ...
                    <tbody class="divide-y divide-gray-800/60">
                        @forelse($aktivitasTerbaru as $a)
                            <tr>
                                ...
                                <td class="py-3 text-gray-500 font-mono text-xs">{{ $a->mataKuliah->kode ?? '-' }}</td>
                                <td class="py-3">
                                    <span class="px-2.5 py-1 rounded-md text-xs font-medium {{ $statusColor[$a->status] ?? 'bg-gray-500/10 text-gray-400' }}">● {{ $a->status }}</span>
                                </td>
                                <td class="py-3 text-gray-400 font-mono text-xs">{{ $a->waktu_masuk ? \Carbon\Carbon::parse($a->waktu_masuk)->format('H.i') : '–' }}</td>
                            </tr>
                        @empty
                            <tr><td colspan="5" class="py-6 text-center text-gray-500">Belum ada aktivitas presensi.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    <section id="tab-tandai" class="tab-section hidden space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-gray-500 mt-1">
                    ...
                    <span id="progressLabel">{{ $ditandaiHariIni }} dari {{ $kelasHariIni->count() }} ditandai</span>
                </p>
            </div>
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-xl p-6">
            <div class="flex items-center justify-between text-sm mb-2">
                ...
                <span id="progressFraction" class="text-gray-400">{{ $ditandaiHariIni }}/{{ $kelasHariIni->count() }}</span>
            </div>
            <div class="h-2 bg-gray-800 rounded-full overflow-hidden">
                <div id="progressBar" class="h-full bg-blue-500 rounded-full transition-all"
                     style="width: {{ $kelasHariIni->count() > 0 ? round($ditandaiHariIni / $kelasHariIni->count() * 100) : 0 }}%"></div>
            </div>
        </div>

        <div class="flex flex-wrap gap-4 text-xs text-gray-400">
            <span class="flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-emerald-500"></span> Hadir (tepat waktu)</span>
            <span class="flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-amber-500"></span> Terlambat (&gt;15 menit setelah mulai)</span>
            <span class="flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-gray-500"></span> Tidak hadir (tidak menekan tombol)</span>
        </div>

        <div class="space-y-4">
            @forelse($kelasHariIni as $i => $k)
                @php $presensi = $presensiHariIni->get($k->id); @endphp
                <div class="bg-gray-900 border border-gray-800 rounded-xl p-5 flex items-center justify-between gap-4 flex-wrap" data-kelas-card="{{ $k->id }}">
                    <div class="flex items-start gap-4">
                        <span class="w-9 h-9 rounded-lg bg-gray-800 flex items-center justify-center text-xs text-gray-400 shrink-0">{{ str_pad($i + 1, 2, '0', STR_PAD_LEFT) }}</span>
                        <div>
                            ...
                            <p class="text-xs text-gray-500 mt-1.5 flex items-center gap-3 flex-wrap">
                                <span>🕐 {{ \Carbon\Carbon::parse($k->jam_mulai)->format('H.i') }} – {{ \Carbon\Carbon::parse($k->jam_selesai)->format('H.i') }}</span>
                                <span>📍 {{ $k->ruang }}</span>
                                @if($k->dosen)<span>{{ $k->dosen }}</span>@endif
                            </p>
                        </div>
                    </div>

                    @if($presensi)
                        <span class="px-3 py-1.5 rounded-md text-xs font-medium {{ $statusColor[$presensi->status] ?? '' }}">● {{ $presensi->status }}</span>
                    @else
                        <button class="btn-tandai px-4 py-2 rounded-lg text-sm font-medium bg-blue-600 hover:bg-blue-500 transition text-white" data-id="{{ $k->id }}">
                            Tandai Hadir
                        </button>
                    @endif
                </div>
    ...
    <section id="tab-riwayat" class="tab-section hidden space-y-6">
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-xl p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
                <p class="text-sm text-blue-400 font-medium">Tingkat Kehadiran Keseluruhan</p>
                <p class="text-4xl font-bold mt-1">{{ $tingkatKehadiran }}<span class="text-xl text-blue-400">%</span></p>
                <p class="text-sm text-blue-400 mt-1">
                    Status: {{ $tingkatKehadiran >= 90 ? 'Sangat Baik' : ($tingkatKehadiran >= 75 ? 'Baik' : 'Perlu Perhatian') }}
                </p>
            </div>
        ...
        <div class="inline-flex bg-gray-900 border border-gray-800 rounded-lg p-1">
            <button data-subtab="permatkul" class="subtab-btn px-4 py-1.5 rounded-md text-sm font-medium text-gray-400 transition">Per Mata Kuliah</button>
            <button data-subtab="lengkap" class="subtab-btn px-4 py-1.5 rounded-md text-sm font-medium text-gray-400 transition">Riwayat Lengkap</button>
        </div>

        <div id="subtab-permatkul" class="subtab-panel bg-gray-900 border border-gray-800 rounded-xl overflow-x-auto">
            <table class="w-full text-sm">
                <thead>
                    <tr class="text-left text-xs text-gray-500 uppercase border-b border-gray-800">
                            ...
                            <td class="p-4">
                                <div class="flex items-center gap-2">
                                    <div class="w-16 h-1.5 bg-gray-800 rounded-full overflow-hidden">
                                        <div class="h-full {{ $r->persentase >= 90 ? 'bg-emerald-500' : ($r->persentase >= 75 ? 'bg-amber-500' : 'bg-red-500') }}" style="width:{{ $r->persentase }}%"></div>
                                    </div>
                                    <span class="{{ $r->persentase >= 90 ? 'text-emerald-400' : ($r->persentase >= 75 ? 'text-amber-400' : 'text-red-400') }}">{{ $r->persentase }}%</span>
                                </div>
                            </td>
                            <td class="p-4">
                                <span class="px-2.5 py-1 rounded-md text-xs font-medium {{ $r->keterangan == 'Aman' ? 'bg-emerald-500/10 text-emerald-400' : ($r->keterangan == 'Memuaskan' ? 'bg-amber-500/10 text-amber-400' : 'bg-red-500/10 text-red-400') }}">{{ $r->keterangan }}</span>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
        ...
        <div id="subtab-lengkap" class="subtab-panel hidden bg-gray-900 border border-gray-800 rounded-xl overflow-x-auto">
            <table class="w-full text-sm">
                <thead>
                    <tr class="text-left text-xs text-gray-500 uppercase border-b border-gray-800">
                ...
                <tbody class="divide-y divide-gray-800/60">
                    @forelse($riwayat->sortByDesc('tanggal') as $item)
                        <tr>
                            ...
                            <td class="p-4 text-gray-500 font-mono text-xs">{{ $item->mataKuliah->kode ?? '-' }}</td>
                            <td class="p-4">
                                <span class="px-2.5 py-1 rounded-md text-xs font-medium {{ $statusColor[$item->status] ?? '' }}">● {{ $item->status }}</span>
                            </td>
                            <td class="p-4 text-gray-400 font-mono text-xs">{{ $item->waktu_masuk ? \Carbon\Carbon::parse($item->waktu_masuk)->format('H.i') : '–' }}</td>
                        </tr>
                    @empty
                        <tr><td colspan="5" class="p-6 text-center text-gray-500">Belum ada riwayat.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <section id="tab-jadwal" class="tab-section hidden space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
            @foreach($urutanHari as $hari)
                <div class="bg-gray-900 border {{ $hari === $hariIni ? 'border-blue-500' : 'border-gray-800' }} rounded-xl overflow-hidden">
                    <div class="px-4 py-3 flex items-center justify-between {{ $hari === $hariIni ? 'bg-blue-500/10' : 'bg-gray-800/40' }}">
                        <span class="font-semibold text-sm {{ $hari === $hariIni ? 'text-blue-400' : '' }}">{{ $hari }}</span>
                        @if($hari === $hariIni)
                            <span class="text-[10px] bg-blue-600 text-white px-2 py-0.5 rounded-full">Hari Ini</span>
                        @endif
                    </div>
                    <div class="p-3 space-y-3">
                        @forelse($jadwalMingguan->get($hari, collect()) as $k)
                            <div class="border border-gray-800 rounded-lg p-3">
                                <p class="font-medium text-sm">{{ $k->nama }}</p>
                                <p class="text-xs text-gray-500 font-mono mt-0.5">{{ $k->kode }}</p>
                                <p class="text-xs text-gray-500 mt-1.5">{{ \Carbon\Carbon::parse($k->jam_mulai)->format('H.i') }} – {{ \Carbon\Carbon::parse($k->jam_selesai)->format('H.i') }}</p>
    ...
    document.querySelectorAll('.btn-tandai').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            const response = await fetch("{{ route('siswa.presensi.store') }}", {
                    ...
                    body: JSON.stringify({ mata_kuliah_id: btn.dataset.id }),
                });
                ...
                if (result.success) {
                    ...
                    btn.outerHTML = `<span class="px-3 py-1.5 rounded-md text-xs font-medium ${badgeClass}">● ${result.status}</span>`;
